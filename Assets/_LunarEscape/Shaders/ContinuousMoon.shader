Shader "LunarEscape/Continuous Moon"
{
    Properties
    {
        _MoonRadius("Moon radius metres",Float)=1737400
        _SurfaceAngle("Surface travel degrees",Float)=0
        _RenderScale("Presentation scale",Float)=1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 _BaseColor,_BaseMap_ST;float _UseMap,_NoiseScale,_Visibility,_InvertFade,_MoonRadius,_SurfaceAngle,_RenderScale;
            CBUFFER_END
            ...
            half4 Frag(Varyings input):SV_Target
            {
                ...
                // 全部层级使用同一球面坐标和材质。局部地形与全球表面边界不会换贴图。
                float3 radial=normalize(input.local-float3(25,-_MoonRadius,0));
                float angle=radians(_SurfaceAngle),sn=sin(angle),cs=cos(angle);
                radial=float3(radial.x*cs-radial.y*sn,radial.x*sn+radial.y*cs,radial.z);
                // 起点选在非极区，避免低空时经纬纹理在极点挤成放射条纹。
                float3 mapped=float3(radial.y,-radial.x,radial.z);
                float2 uv=float2(atan2(mapped.z,mapped.x)/6.2831853+.5,asin(clamp(mapped.y,-1,1))/3.14159265+.5);
                float2 mapPoint=uv*float2(10916.4,5458.2);
                float metresPerPixel=length(fwidth(input.local.xz));
                float grain=(Noise(input.local.xz*.26)-.5)*.18*(1-smoothstep(.3,4,metresPerPixel));
                grain+=(Noise(input.local.xz*2.3)-.5)*.10*(1-smoothstep(.03,.7,metresPerPixel));
                float3 albedo=_BaseColor.rgb*SAMPLE_TEXTURE2D(_BaseMap,sampler_BaseMap,uv).rgb*2.2*(1+grain);
                float3 normal=normalize(input.normalWS);
                // 程序坑只补全球低分辨率影像的远景光照，不改变基地附近的碰撞形状。
                float detailBlend=smoothstep(1400,5000,distance(input.positionWS,_WorldSpaceCameraPos)/max(.0001,_RenderScale));
                float2 slope=CraterSlope(mapPoint,2.4,.075)*(1-smoothstep(.3,1.2,length(fwidth(mapPoint))))+CraterSlope(mapPoint,13,.055);
                float3 tangent=normalize(TransformObjectToWorldDir(float3(1,0,0)));
                float3 bitangent=normalize(TransformObjectToWorldDir(float3(0,0,1)));
                normal=normalize(normal-detailBlend*(slope.x*tangent+slope.y*bitangent));
