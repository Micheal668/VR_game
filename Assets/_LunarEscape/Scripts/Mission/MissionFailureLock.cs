...
{
    // 死亡结算的行动边界：停游戏操作，保留头手追踪与结算 UI。
    // 不使用 Time.timeScale=0，避免把 XR 输入和重新开始一起暂停。
    [DefaultExecutionOrder(10000)]
    public sealed class MissionFailureLock : MonoBehaviour
    {
        ...
        public void Configure(StationMission task, XROrigin origin, GameObject summary)
        {
        ...
        {
            if (phase == StationMissionPhase.Failed && !IsLocked) Lock();
            else if (phase == StationMissionPhase.Briefing && IsLocked) Restore();
        }
        ...
        private void Lock()
        {
            ...
            foreach (var raycaster in SceneComponents<BaseRaycaster>())
                if (!raycaster.transform.IsChildOf(summaryRoot.transform)) Disable(raycaster);
            foreach (var control in SceneComponents<Selectable>())
            {
                if (control.transform.IsChildOf(summaryRoot.transform)) continue;
